...
export async function POST(request: NextRequest) {
  ...
  // Текст — шаблон assignmentNew из админки ({name} — игрок, {coach} — тренер).
  const templates = await getPushTemplates();
  const coach = `${auth.user.firstName ?? ''} ${auth.user.lastName ?? ''}`.trim();
  Promise.allSettled(
    filtered.map((athleteId) =>
      sendUserPush(athleteId, assignmentNewPush(templates, { name: memberById.get(athleteId)?.firstName, coach })),
    ),
  ).catch(() => { });
